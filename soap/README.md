...
(4) Update `log4j2.xml` file under `src/main/resources` by renaming all the occurrences of `soap-api-template-v1` in the `<RollingFile>` element with the same name used for the project's root folder above (e.g. `my-new-api-v1`). Leave all the remaining parts of `log4j2.xml` unchanged and just update the `soap-api-template-v1` occurrences where they appear in the file. See picture below for an example.
...
api.version=v1
api.application.name=soap-api-template
```
